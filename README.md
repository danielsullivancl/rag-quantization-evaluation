...
To compute mean values per configuration and generate the LaTeX table used in the paper, run:
...
To run Mann-Whitney U tests comparing quantization levels and RAG configurations, run:

    python scripts/compute_hypothesis_tests.py

This script generates:

- `results/hypothesis_mannwhitney.csv`
- `results/hypothesis_mannwhitney.tex`

## Generating Violin Plots

To generate violin plots showing the distribution of the main metrics, run:
...
The experiment uses a fixed temperature of 0 to reduce stochastic variation in model generation. Each question is executed once per configuration. The variability observed in the results reflects differences across the 450 questions in the workload, not repeated executions of the same question.
